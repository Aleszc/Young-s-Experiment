...
public void actionPerformed(ActionEvent e) 
{
  		...
  		else if(e.getSource() == i2  && panelslider.b1.isSelected()==true) 
  		{
        ...
        try {
  		...
  		else if(e.getSource() == i2  && panelslider.b2.isSelected()==true) 
  		{
		...
		public void stateChanged(ChangeEvent e) {//zmienianie linii w panelu glownym
			...
			for(int i =  1 +(int)(0.5*panel.X); i<(int)(0.9*panel.X);i++) 
			{
				...
				sb2.append("\n");
				series2.add(0, 1);
				for(int i=1 + (int)(0.5*panel.lista.size());i<panel.lista.size();i++)
	    		{
				 	...
				 	sb.append(theta1*180/Math.PI);  
					sb.append("                ");
					sb.append(height1/100);  
					sb.append("\n");
					
				 	
	    		}
				for(int i=0;i<panel.lista.size();i++)
	    		{
				 	double theta1= Math.atan(Math.abs(0.5*panel.X-i)/(0.67*panel.Y));
				 	double bheta1 = 0.1*((Math.PI * panel.D*1000)/(panel.lambda))*(Math.sin(theta1));
				 	double height1 = (100*((Math.sin(bheta1))/(bheta1))*((Math.sin(bheta1))/(bheta1)));
